...
Run only this feature from the repository root:

```sh
pnpm exec cucumber-js --config cucumber.mjs --tags '@session-setup'
```

Use `pnpm test:bdd:dry` to check discovery and `pnpm test:bdd` for the full suite. Dry runs do not prove
behavior; the session suite must fail until its bindings and implementation are complete. Use real
request/store integration for ordering, cookie protections, expiry, and failures; do not add diagnostic UI.
Keep per-scenario state isolated and preserve existing tenant bindings and resource cleanup.
