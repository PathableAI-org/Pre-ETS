...
| Invalid   | Missing path, empty path, or non-directory → configuration unavailable     |
...
1. Process starts; mode selected; filesystem source bound to `TENANT_CONFIG_DIR`.
2. Request binds alias (host) or uses static alias.
3. Source opens at most one file for that alias.
4. Success returns `TenantConfig` to existing consumers; failure maps as above.
5. File edits require process restart (or documented reload) to refresh cached parses.
