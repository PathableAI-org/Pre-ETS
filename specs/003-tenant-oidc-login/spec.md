...
- **FR-001**: Frontend-owned tenant configuration MUST retain Display Name and carry required OIDC login settings alongside it—not replacing Display Name. Those settings MUST include OIDC issuer identity, application client identifier, a required trusted client registration mode (`public` or `confidential` / planned `clientAuth`), and broker connection selection where needed to reach that tenant's login experience. Tenant identity MUST remain the existing tenant id; no new tenant identification mechanism is introduced. Registration mode MUST NOT be inferred from provider discovery.
...
- **Tenant configuration**: Existing frontend-owned shared configuration associated with one tenant id. Display Name remains a required tenant field; required OIDC login settings (issuer, client, registration mode / `clientAuth`, and connection when needed) sit alongside Display Name in the same configuration—they do not replace it. Absence of usable `oidc` settings is invalid login configuration (HTTP 403 extended forbidden), not a Display Name-only success path.
- **OIDC login settings**: Trusted issuer and client identity, required registration mode (`public` or `confidential`), any required broker connection selection, and access to required server-only credentials when confidential. These fields are peers of Display Name in tenant configuration. A connection selects the tenant's identity provider through the broker; tenant SAML metadata remains broker-owned.
...
- The current [tenant specification](../001-tenant-resolution/spec.md) and frontend tenant contract establish Display Name, tenant slug identity, host-associated resolution, and explicit bare-localhost static development mode. This feature extends the earlier slice's Display Name-only scope and changes unauthenticated `/` document navigation from Display Name landing to login initiation (or failure). Shared tenant configuration continues to carry Display Name and adds required OIDC fields beside it; a missing or unusable `oidc` object is invalid login configuration and uses HTTP 403 extended forbidden (not `login-unavailable`).
