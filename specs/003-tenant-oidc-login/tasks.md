...
**Purpose**: Extended tenant OIDC schema (`clientAuth`), secrets map rules, and OIDC module seams
every story needs. **Do not enable Proxy login redirection until this phase is complete.**

- [ ] T005 Write failing unit cases in `packages/frontend/tests/unit/tenant-oidc-config.test.ts` for
      `TenantConfig` / `oidc` parse rules in T006: required nested `oidc`; `issuer` absolute URL with
      https-outside-dev / loopback-http-in-dev; nonempty `clientId`; required `clientAuth` closed set
      `"public"` \| `"confidential"`; optional nonempty `connection`; reject unknown keys; Display
      Name-only unusable for login (FR-001, FR-002, FR-007, data-model, `contracts/tenant-oidc-config.md`).
- [ ] T006 Extend `packages/frontend/src/lib/tenant/types.ts` (and parsers used by
      `source.ts` / operations) so `TenantConfig` includes `oidc` with fields and constraints from
      T005; keep restart/immutability semantics; make T005 pass (FR-001).
- [ ] T007 [P] Write failing unit cases in `packages/frontend/tests/unit/oidc-secrets.test.ts` for
      `OIDC_CLIENT_SECRETS_JSON`: lazy parse; malformed map → typed process failure; `"public"` allows
      absent secret; `"confidential"` requires nonempty slug secret else config refusal; never log
      secret values (FR-002, research §2).
- [ ] T008 [P] Implement `packages/frontend/src/lib/oidc/secrets.ts` (`resolveOidcClientSecret(slug,
      clientAuth)`) per T007 and `contracts/tenant-oidc-config.md`; make T007 pass.
- [ ] T009 Write failing unit cases in `packages/frontend/tests/unit/oidc-types.test.ts` for
      ...
      default `pre-ets:oidc-tx:`, and correlation cookie claims `state`/`tenant`/`exp` (data-model).
- [ ] T010 Define OIDC types/helpers in `packages/frontend/src/lib/oidc/types.ts` per T009; parse
      `OIDC_TX_*` lazily; make T009 pass.
- [ ] T011 [P] Implement Redis transaction helpers in `packages/frontend/src/lib/oidc/transaction.ts`:
...
- [ ] T012 [P] Implement correlation cookie sign/verify in `packages/frontend/src/lib/oidc/cookie.ts`
      (cookie name `pathable-oidc`; HS256 via `jose`; `OIDC_TX_SIGNING_SECRET` or
      `SESSION_SIGNING_SECRET`; host-only HttpOnly Lax; Secure outside development) (FR-006).
- [ ] T013 [P] Implement discovery + in-process issuer cache in
...
- [ ] T014 Implement `initiateLogin` in `packages/frontend/src/lib/oidc/initiate.ts` per
      ...
      401-nondoc without attaching session cookie on failures (FR-003–FR-007).

**Checkpoint**: Unit contracts for config/secrets/tx/cookie/discovery pass; clean tree
builds/typechecks without OIDC secrets; Proxy still does not redirect to IdP.
...
- [ ] T015 [P] [US1] Add failing Proxy/initiation unit cases in
      `packages/frontend/tests/unit/oidc-initiate-proxy.test.ts` (or extend existing proxy tests):
      unauthenticated `reuse` and `create` both initiate; terminals 403/500/503 unchanged; successful
      document `create` 302 sets `pathable-session` + `pathable-oidc`; failures and nondoc `401` omit
      `pathable-session` Set-Cookie; caller query cannot override issuer/client/connection/return host;
      PKCE verifier absent from Location; early Next 16.3.5 RSC/`Accept` signal for nondoc `401`
      recorded (FR-003–FR-006, FR-013, E2, E4).
- [ ] T016 [P] [US1] Replace pending US1 stubs in `tests/bdd/steps/oidc.steps.ts` for
...
- [ ] T017 [US1] Wire `packages/frontend/src/proxy.ts`: after `setupSession`, unauthenticated ready
      → `initiateLogin`; matcher covers `/` and `/auth/callback` (callback pass-through only); never
      SSR landing without authenticated user id (out of scope / always initiate); safe diagnostics
      outcome classes only (FR-003, FR-005, FR-013, E7, E9).
- [ ] T018 [US1] Add `packages/frontend/src/app/auth/callback/page.tsx` stub outside login re-entry;
      no code exchange; no initiation (FR-013).
- [ ] T019 [US1] Add `packages/frontend/src/app/login-unavailable/page.tsx` **outside** `(app)` per
...
- [ ] T020 [US1] Apply **Supersedes** updates in the same delivery slice as Proxy initiation:
...
- [ ] T021 [US1] Same-slice **behavioral supersession** retarget for inventoried features (plan
...
- [ ] T022 [US1] Run US1 partition via `pnpm test:bdd:oidc` (and unit suite); prove two-tenant
      isolation, PKCE/tx binding, reuse re-initiation, cookie-absence on failures; record gaps in
      `specs/003-tenant-oidc-login/quickstart.md` (SC-001–SC-003, SC-005).
...
**Goal**: Trusted OIDC settings (incl. `clientAuth`) drive destinations; config defects use extended
403; provider/tx failures use `login-unavailable`; secrets stay server-only.
...
- [ ] T023 [P] [US2] Add failing unit/HTTP cases for config defects and confidential credentials in
      ...
      secret; secret never in redirects/HTML/diagnostics; connection omit vs required (FR-001, FR-002,
      FR-004, FR-007, FR-008, FR-009).
- [ ] T024 [P] [US2] Replace pending US2 stubs in `tests/bdd/steps/oidc.steps.ts` for
...
- [ ] T025 [US2] Complete extended forbidden UX for OIDC-config 403 on the existing forbidden surface
      (no new route): explain login cannot start; clear next action; a11y; no secrets; distinct from
      `login-unavailable` (FR-007, FR-008, P4).
- [ ] T026 [US2] Ensure provider metadata / discovery failures route to `login-unavailable` (T019)
      without collapsing into 403; no auto redirect loop (FR-007, US2 scenario 6).
- [ ] T027 [US2] Document reload/restart procedure for OIDC JSON + secrets in
      `packages/frontend/.env.example` and operator-facing docs pointers (FR-009, FR-011).
- [ ] T028 [US2] Run US2 cases via `pnpm test:bdd:oidc` and record evidence in
...
- [ ] T029 [P] [US3] Replace pending US3 stubs in `tests/bdd/steps/oidc.steps.ts` for
...
- [ ] T030 [US3] Finish local provisioning docs in `docs/docker-compose.md`, README, and
...
- [ ] T031 [US3] Ensure BDD fixtures can supply synthetic OIDC tenant JSON + optional secrets map and
      unique `SESSION_KEY_PREFIX` / `OIDC_TX_KEY_PREFIX` for spawned frontends (`tests/bdd/support/**`)
      (FR-010, FR-012).
- [ ] T032 [US3] Document CI Keycloak policy (E8) in `.github/workflows/ci-bdd.yml` comments and/or
      ...
      added (plan Testing).
- [ ] T033 [US3] Run US3 local/quickstart checks and `features/local-oidc-development.feature` where
...
- [ ] T034 [P] Sync `docs/authentication.md` with unauthenticated initiate-for-both, `clientAuth`,
      cookie-on-success-only, and mid-journey (provider arrival only) claims (research §11).
- [ ] T035 Require OIDC delivery PR keeps `pnpm test:bdd:session` green **after** T021 retarget (E5);
      confirm path filters cover `packages/frontend/src/lib/oidc/**` and `proxy.ts`.
- [ ] T036 Run `pnpm test:bdd:dry`, `pnpm --filter @pathableai/pre-ets-frontend test:unit`,
...
- [ ] T037 Run `pnpm typecheck`, `pnpm build`, `pnpm lint`, `pnpm format:check`, `pnpm check:unused`
...
- **Foundational (Phase 2)**: Depends on Setup — **BLOCKS** all user stories. T005→T006; T007→T008;
  T009→T010; T011–T013 can proceed in parallel after T010 types exist; T014 after secrets + discovery
  - tx + cookie seams.
- **User Story 1 (Phase 3)**: Depends on Foundational. MVP = Setup + Foundational + US1 (through
  T022). T021 retarget is same-slice as Proxy enablement (T017)—do not ship initiation without it.
- **User Story 2 (Phase 4)**: Depends on Foundational; shares Proxy/initiation with US1; independently
  testable via `tenant-oidc-configuration.feature`.
...
- **US1 (P1)**: After Foundational — MVP demo of provider-page arrival for two tenants.
- **US2 (P1)**: After Foundational — config/`clientAuth`/403 taxonomy; Constitution III completeness
  for tampering and secret isolation expects US2 (T023–T028).
...
- T007/T008 parallel with T005/T006 once schemas agreed
- T011/T012/T013 after T010
- T015/T016 parallel; T023/T024 parallel
...
```bash
# After Foundational:
Task: "T015 failing Proxy/initiation unit cases in packages/frontend/tests/unit/oidc-initiate-proxy.test.ts"
Task: "T016 failing US1 stubs in tests/bdd/steps/oidc.steps.ts for features/tenant-oidc-login.feature"

# Then implement T017–T021; validate with T022
```
...
3. Complete Phase 3: User Story 1 (including same-slice Gherkin retarget T021)
4. **STOP and VALIDATE**: two-tenant initiation without landing
5. Demo provider-page arrival

### Incremental Delivery

1. Setup + Foundational → seams ready
2. US1 → MVP initiation
3. US2 → config defects / confidential / extended 403
...
2. Dev A: US1 Proxy/initiation; Dev B: US2 config/failure UX (after T014 exists); Dev C: US3 docs/fixtures
...
- Never-land and `clientAuth` are non-negotiable product/plan rules
