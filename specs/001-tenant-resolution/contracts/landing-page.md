...
comes from `getCurrentTenantConfig()` and is rendered through the existing PathAble `Text` component as ordinary text.
...
| Unknown/invalid host in host mode                            | Actual 403 refusal, no successful landing page or tenant name.            |
...
3. Visit unknown hosts and bare localhost in host mode. Assert actual response status 403 and absence of either known
   name; do not accept redirects, a styled error inside a 200 document, or a default tenant.
