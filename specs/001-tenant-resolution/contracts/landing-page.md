...
Display Name landing obligations in this file apply only after an **authenticated user id** exists on
the session (callback / authenticated SSR—out of scope in 003). Until that field exists, treat
unauthenticated `/` as initiate-or-fail per the 003 initiation contract.

Dual-layer regression intent (once authenticated landing exists):

1. Unauthenticated `/` → initiate OIDC or fail (003)
2. Authenticated `/` → may SSR Display Name per this contract (001)

## Successful visit (authenticated identity only)

When the session carries an authenticated user id, the landing page displays
`Tenant: {Display Name}` directly after its current top-level heading. Display Name comes from
`getCurrentTenantConfig(tenant)` / the session snapshot and is rendered through the existing PathAble
`Text` component as ordinary text. The page remains server-rendered. The name is visible in the
document and available to assistive technology without waiting for a browser-side fetch or user
action.
...
| Known host with authenticated identity, production           | `Tenant: Springfield Demo` / Shelbyville Demo from that record (later slice).                                                                                                                          |
| Known host `springfield.localhost`, authenticated, host mode | Same Display Name contract using local known records (later slice).                                                                                                                                    |
| Bare `localhost`, static mode, authenticated                 | The supplied static Display Name; no claim that Host established tenancy (later slice).                                                                                                                |
...
2. **Authenticated Display Name (later)**: After authenticated identity exists, assert configured
   Display Names, reload isolation, and overlapping visits as in the original 001 journeys.
