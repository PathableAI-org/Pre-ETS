...
describe("recordQualifyingActivity", () => {
  ...
  it("accepts deliberate activity before the idle deadline and leaves expiresAt unchanged", async () => {
    ...
    const store = mockStore({
      renewIdleActivity: vi.fn().mockResolvedValue({ kind: "renewed", record: renewed })
    })
    ...
    expect(vi.mocked(store.renewIdleActivity)).toHaveBeenCalledWith(
      sessionId,
      now + 120,
      "springfield"
    )
  ...
  it("derives session identity only from the verified cookie (no caller-supplied sid)", async () => {
    ...
    expect(vi.mocked(store.renewIdleActivity).mock.calls[0]?.[0]).toBe(sessionId)
    expect(vi.mocked(store.renewIdleActivity).mock.calls[0]?.[2]).toBe("springfield")
  })
  ...
  it("denies missing cookie without calling the store", async () => {
    ...
    expect(vi.mocked(store.renewIdleActivity)).not.toHaveBeenCalled()
  })
