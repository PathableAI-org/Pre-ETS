...
describe("idle expiration HTTP / setup clocks", () => {
  ...
  it("denies authenticated reuse at idleExpiresAt and clears for inactivity", async () => {
    ...
    const clearForInactivity = vi.fn().mockResolvedValue({ kind: "cleared", record: cleared })
    ...
    const store = mockStore({ clearForInactivity, create, read })
  ...
  it("slow Redis read that crosses idleExpiresAt fails closed with clearance", async () => {
    ...
    const events: string[] = []
    const store = mockStore({
      clearForInactivity: vi.fn().mockImplementation(() => {
        events.push("clear")
        return Promise.resolve({ kind: "cleared" as const, record: cleared })
      }),
  ...
  it("does not expose a public diagnostic idle endpoint surface", () => {
    // Contract pin: activity is a Server Action / protected path — no GET probe module.
