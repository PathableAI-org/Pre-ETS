...
describe("guardAuthenticatedAccess", () => {
  ...
  it("samples a fresh clock after Redis load before success", async () => {
    ...
    expect(clearForInactivity).toHaveBeenCalled()
  })
  ...
  it("does not rewrite idleDurationMinutes when tenant policy would differ", async () => {
    const stampedDuration = 15
    const record = idleAuthenticatedRecord({
      idleDurationMinutes: stampedDuration,
      idleExpiresAt: computeIdleExpiresAt(1_700_000_000, stampedDuration)
    })
    ...
    const store = mockStore({ read, update })

    const result = await guardAuthenticatedAccess(
      { sessionId: fixedSessionId(13), tenantId: "springfield" },
      { nowSeconds: () => 1_700_000_060, store }
    )
